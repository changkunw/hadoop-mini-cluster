...
public class MyServer4 {
    ...
    public static final String FILE_DIR = "G:\\hadoopdata\\xxx\\";
    ...
    private static void doit(final ServerSocketChannel serverSocketChannel) throws IOException {
        ...
        try {
            ...
            //            sendFile(socketChannel, new File("G:\\hadoopdata\\xxx_server_receive.txt"));
        } finally {
            try {
                socketChannel.close();
            } catch (Exception ex) {
    ...
    private static String receiveFile(SocketChannel socketChannel) throws IOException {
        FileOutputStream fos = null;
        FileChannel channel = null;
        String filePath;
        try {
            ByteBuffer fileNameByteBuffer = ByteBuffer.allocate(1024);
            socketChannel.read(fileNameByteBuffer);
            String fileNameString = new String(fileNameByteBuffer.array());
            fileNameByteBuffer.clear();
            System.out.println(fileNameString);
            filePath = FILE_DIR + File.separator + fileNameString.substring(0, fileNameString.indexOf("@"));
